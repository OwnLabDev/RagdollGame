...
public class TransformReset : DecoratorEditor
{
    ...
    Vector3 StringToVector3(string sVector)
    {
        ...
        //// split the items
        //string[] sArray = sVector.Split(',', ' ').Select(s => s.Trim()).ToArray();

        //Debug.Log($"{sVector}, {sArray[0]}");
    ...
    public static Vector3 Parse(string str) {
        ...
        return new Vector3(float.Parse(s[0], culture), float.Parse(s[1], culture), float.Parse(s[2], culture));
    }
}
